...
module.exports = {
    ...
    determineBumpType: function (opts, specificHash) {
        ...
        exec(gitCheckContains, function (err, stdout, stderr) {
            if (err) {
                console.log('Git Error: Could not read branches containing this commit');
                deferred.reject(new Error('Git Error: Could not read branches containing this commit'));
            }
            ...
            if (!isOnMaster && opts.masterOnly) {
                console.log('This commit is not contained on master. Set `masterOnly` to false to disable this.');
                return deferred.reject(new Error('You are currently not on master.'));
            } else {
                ...
                console.log(((foundBranch !== null) ? 'Found on ' + foundBranch + '. ' : 'This commit is not contained in a special branch. ') + 'Bump as ' + bumpAs + '.');
            }
